...
if($hasil->num_rows > 0) { 
 ...
 while($row = $hasil->fetch_array()) {
 	...
 	// active 
 	if($row[4] == 1) { 		
 		$paymentStatus = "<label class='label label-success'>Bayar Penuh</label>";
 	} else if($row[4] == 2) { 		
 		$paymentStatus = "<label class='label label-info'>Bayar Kredit</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-warning'>Belum Bayar</label>";
 	} // /else
